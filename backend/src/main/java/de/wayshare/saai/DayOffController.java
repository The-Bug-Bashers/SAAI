  ...
  public ResponseEntity<String> dayOffRequest(@RequestBody Map<String, String> body) {
    ...
    if (username == null || verificationNumberStr == null) {
      ...
      return new ResponseEntity<>(
          "Error: Missing username or verification number", HttpStatus.BAD_REQUEST);
    }
    ...
    try {
      ...
      return new ResponseEntity<>(
          "Error: Invalid verification number format", HttpStatus.BAD_REQUEST);
    }
    ...
    logger.info(
        "Calculated correct verification number: {} for username: {}",
        correctVerificationNumber,
        username);

    if (verificationNumber != correctVerificationNumber) {
      logger.warn(
          "Incorrect verification number provided for username: {}. Correct number: {}",
          username,
          correctVerificationNumber);
    ...
    try {
      ...
      return new ResponseEntity<>(
          "Error: Failed to retrieve InfoScreen events", HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    if (userEvents.isEmpty()) {
      ...
      return new ResponseEntity<>(
          "Error: Verification successful, but no events found for the user", HttpStatus.NOT_FOUND);
    }
    ...
    for (Map<String, Object> event : userEvents) {
      ...
      try {
        ...
        return new ResponseEntity<>(
            "Error: Failed to fetch event details", HttpStatus.INTERNAL_SERVER_ERROR);
      }

      if (!eventResponse.getStatusCode().is2xxSuccessful()) {
        logger.error(
            "Non-successful response for fetching event details, status: {}",
      ...
      for (Map<String, Object> user :
          (List<Map<String, Object>>) eventData.get("responsible_users")) {
        String foundUsername = (String) user.get("username");
        if (foundUsername != null && foundUsername.equals(username)) {
      ...
      if (requestingUserUUID == null) {
        ...
        return new ResponseEntity<>(
            "Error: No matching UUID found for the user", HttpStatus.BAD_REQUEST);
      }

      // Update responsible_users list by removing the requesting user UUID
      List<Map<String, Object>> responsibleUsers =
          (List<Map<String, Object>>) eventData.get("responsible_users");
      ...
      for (Map<String, Object> user : responsibleUsers) {
        ...
        if (!userUUID.equals(requestingUserUUID)) {
          ...
          logger.info(
              "Removing user {} (UUID: {}) from event UUID: {}", username, userUUID, eventUUID);
        }
      }
