...
public class SaniAlarmApiTokenService {
  ...
  public SaniAlarmApiTokenService(SaniAlarmApiConfig config, RestTemplate template) {
    this.config = config;
  ...
  public String getToken() {
    if (isTokenValid()) {
      logger.debug(
          ...
          cachedToken,
          Instant.now().until(tokenExpiry, ChronoUnit.SECONDS));
      return cachedToken;
    }
    ...
    if (!response.getStatusCode().is2xxSuccessful()) {
      throw new TokenRetrievalException(
          "Response status: " + response.getStatusCode() + " Body: " + response.getBody());
    }

    TokenResponse responseBody = response.getBody();
    if (responseBody == null
        || responseBody.access_token() == null
        || responseBody.access_token().isEmpty()
    ...
    tokenExpiry =
        Instant.now().plusSeconds(responseBody.expires_in() - config.tokenExpiryBufferSeconds());
    if (Instant.now().isAfter(tokenExpiry)) {
      throw new TokenRetrievalException(
          "Calculated token expiry is already past directly after retrieval");
    }

    logger.info(
        "Token successfully retrieved. Expires in: {} seconds.",
        Instant.now().until(tokenExpiry, ChronoUnit.SECONDS));
    logger.debug(
        "Using new Token: {} Expires in {} seconds.",
        cachedToken,
        Instant.now().until(tokenExpiry, ChronoUnit.SECONDS));
    return cachedToken;
  }
  ...
  private ResponseEntity<TokenResponse> sendNewTokenRequest() {
    ...
    try {
      ...
      throw new TokenRetrievalException(
          "Response status: " + e.getStatusCode() + " Body: " + e.getResponseBodyAsString(), e);
