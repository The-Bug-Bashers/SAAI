...
public class TimetableService {
  ...
  public void generateTimetablesForWeek() {
    ...
    for (int i = 0; i < weekDays.size(); i++) {
      ...
      generateTimetableForDay(
          dutyGroups, userUuidMap, capitalize(dayName.toLowerCase()), currentDay);
    }
  }
  ...
  private void generateTimetableForDay(
      List<Map<String, Object>> dutyGroups,
      Map<String, String> userUuidMap,
      String day,
      LocalDate date) {
    ...
    for (Map.Entry<String, List<Map<String, Object>>> entry : groupsByStartTime.entrySet()) {
      ...
      for (Map<String, Object> group : startTimeGroups) {
        List<String> dutyDays =
            group.get("dutyDays") instanceof List
      ...
      Map<String, Object> selectedGroup =
          Collections.max(
              eligibleGroups,
              Comparator.comparingInt(
                  group ->
                      group.get("daysSinceLastDuty") instanceof Integer
      ...
      String dutyEnd =
          day.equals("Friday") && selectedGroup.get("fridayDutyEnd") != null
              ? selectedGroup.get("fridayDutyEnd").toString()
              : selectedGroup.get("dutyEnd") != null
                  ? selectedGroup.get("dutyEnd").toString()
                  : null;
      ...
      if (!userUuids.isEmpty() && dutyStart != null) {
        ...
        String dutyEndTime =
            day.equalsIgnoreCase("Friday") && selectedGroup.get("fridayDutyEnd") != null
                ? selectedGroup.get("fridayDutyEnd").toString()
                : selectedGroup.get("dutyEnd") != null
                    ? selectedGroup.get("dutyEnd").toString()
                    : null;
        ...
        if (dutyEndTime != null) {
          ...
          sendLiveTickerMessage(
              "Error: Duty end time is missing for group " + selectedGroup.get("id"));
        }
      }

      // Increment daysSinceLastDuty for ALL groups in the same starting time group except the
      // selected group
      for (Map<String, Object> group : startTimeGroups) {
  ...
  private void createTimetableEvent(
      String startDateTime, String endDateTime, List<String> userUuids) {
