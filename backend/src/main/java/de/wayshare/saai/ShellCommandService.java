...
public class ShellCommandService {
  ...
  public void executeSignalCliReceive() {
    try {
      ...
      Process receiveProcess =
          Runtime.getRuntime()
              .exec("signal-cli receive -t 2 --ignore-attachments --ignore-stories");

      // Capture the output of the receive command
      BufferedReader receiveReader =
          new BufferedReader(new InputStreamReader(receiveProcess.getInputStream()));
      String receiveLine;
  ...
  public CompletableFuture<Void> executeSignalCliSend(
      String message, String addressOrPhoneNumber, boolean isGroup) {
    return CompletableFuture.runAsync(
        () -> {
          try {
            // Build the send command dynamically based on whether it's a group or individual phone
            // number
            String[] sendCommand;
            if (isGroup) {
              sendCommand =
                  new String[] {"signal-cli", "send", "-m", message, "-g", addressOrPhoneNumber};
            } else {
              sendCommand =
                  new String[] {"signal-cli", "send", "-m", message, "-u", addressOrPhoneNumber};
            }
            ...
            BufferedReader sendReader =
                new BufferedReader(new InputStreamReader(sendProcess.getInputStream()));
            String sendLine;
