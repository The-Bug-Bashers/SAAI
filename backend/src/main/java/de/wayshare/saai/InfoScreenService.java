...
public class InfoScreenService {
  ...
  private String stripFractionalSeconds(String dateTimeStr) {
    if (dateTimeStr != null && dateTimeStr.contains(".")) {
      return dateTimeStr.substring(0, dateTimeStr.indexOf('.'))
          + dateTimeStr.substring(dateTimeStr.indexOf('+'));
    }
    return dateTimeStr;
  }
  ...
  private List<Map<String, Object>> filterEvents(List<Map<String, Object>> events) {
    ...
    for (Map<String, Object> event : events) {
      ...
      try {
        Date startDateTime =
            localDateFormat.parse(dateFormat.format(dateFormat.parse(startDatetimeStr)));
        Date endDateTime =
            localDateFormat.parse(dateFormat.format(dateFormat.parse(endDatetimeStr)));

        boolean isRepeating =
            event.containsKey("event_metadata")
                && !((List<Map<String, Object>>) event.get("event_metadata")).isEmpty();
        boolean isTodayEvent = startDateTime.before(todayEnd) && endDateTime.after(todayStart);

        if (isTodayEvent
            || (isRepeating
                && isTodayRepeatingEvent(
                    (List<Map<String, Object>>) event.get("event_metadata")))) {
    ...
    for (Map<String, Object> event : todayEvents) {
      List<String> responsibleUsers =
          extractParamedics((List<Map<String, Object>>) event.get("responsible_users"));
      if (!responsibleUsers.isEmpty()) {
        Map<String, Object> formattedEvent = new HashMap<>();
        formattedEvent.put(
            "start_time", formatTime(stripFractionalSeconds((String) event.get("start_datetime"))));
        formattedEvent.put(
            "end_time", formatTime(stripFractionalSeconds((String) event.get("end_datetime"))));
        formattedEvent.put("responsible_users", responsibleUsers);
        formattedEvent.put(
            "is_active",
            isActiveEvent(
                stripFractionalSeconds((String) event.get("start_datetime")),
                stripFractionalSeconds((String) event.get("end_datetime")),
                now));
  ...
  private String getNextActiveStatus(List<Map<String, Object>> formattedEvents) {
    ...
    for (Map<String, Object> event : formattedEvents) {
      Boolean isActive =
          (Boolean) event.get("is_active"); // This field is now in the formatted events
  ...
  private String formatTime(String dateTimeStr) {
    try {
      ...
      outputFormat.setTimeZone(
          TimeZone.getTimeZone("Europe/Berlin")); // convert dates to local time
  ...
  private boolean isActiveEvent(String startDateTimeStr, String endDateTimeStr, Date now) {
    if (startDateTimeStr == null || endDateTimeStr == null) {
      logger.warn(
          "Null datetime found for event. Start: {}, End: {}", startDateTimeStr, endDateTimeStr);
      return false;
    }
    ...
    try {
      ...
      localTimeFormat.setTimeZone(
          TimeZone.getTimeZone("Europe/Berlin")); // Convert to Berlin local time
      ...
      logger.info(
          "Comparing current time (local, time-only): {} with event start: {} and event end: {}",
          currentTimeStr,
          eventStartTimeStr,
          eventEndTimeStr);
      ...
      logger.error(
          "Error parsing event times. Start: {}, End: {}", startDateTimeStr, endDateTimeStr, e);
      return false;
    }
  }
