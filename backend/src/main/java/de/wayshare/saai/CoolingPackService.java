...
public class CoolingPackService {
  ...
  @Autowired private CoolingPackRepository coolingPackRepository;
  ...
  private void validateCoolingPacksPassword(String password) {
    ...
    if (!coolingPacksPassword.equals(password)) {
      throw new ResponseStatusException(
          HttpStatus.UNAUTHORIZED, "Incorrect password for cooling packs");
    }
  }
  ...
  public CoolingPack updateCoolingPackStatus(
      ...
      String givenBy,
      String borrowedBy,
      Integer maxLendingDuration,
      String password) {
    validateCoolingPacksPassword(password); // PUT requires coolingpacks.service.password

    CoolingPack coolingPack =
        coolingPackRepository
            .findById(id)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Cooling pack not found"));
  ...
  public void notifyOverdueItems(String coolingPacksPassword, String usersPassword) {
    ...
    String coolingPacksUrl =
        "https://saai.wayshare.de:9090/api/coolingpacks?password=" + coolingPacksPassword;
    ...
    ResponseEntity<List<Map<String, Object>>> usersResponse =
        restTemplate.exchange(
            usersUrl, HttpMethod.POST, userRequestEntity, new ParameterizedTypeReference<>() {});
    ...
    for (Map<String, Object> pack : coolingPacks) {
      ...
      if (isBorrowed != null && isBorrowed) {
        ...
        if (maxLendingDuration != null
            && borrowedDate.plusDays(maxLendingDuration).isBefore(today)) {
          // Prepare the message
          ...
          String liveTickerMessage =
              String.format(
                  "Overdue item notification sent for '%s' lent by '%s'.", itemName, givenBy);
