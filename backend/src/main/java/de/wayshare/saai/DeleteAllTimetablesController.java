  ...
  public ResponseEntity<String> deleteAllTimetables(@RequestBody Map<String, String> body) {
    ...
    try {
      ...
      return new ResponseEntity<>(
          "Error: Failed to retrieve timetable events", HttpStatus.INTERNAL_SERVER_ERROR);
    }

    if (!timetablesResponse.getStatusCode().is2xxSuccessful()) {
      logger.error(
          "Non-successful response from timetables API: {}", timetablesResponse.getStatusCode());
      return new ResponseEntity<>(
          "Error: Failed to fetch timetable events", HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    for (Map<String, Object> timetable : timetables) {
      ...
      try {
        SimpleDateFormat inputFormat =
            new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssX"); // Corrected format
      ...
      logger.info(
          "DELETE Response for UUID {}: Status: {}, Body: {}",
          uuid,
