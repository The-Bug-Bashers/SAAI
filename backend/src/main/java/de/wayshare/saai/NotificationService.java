...
public class NotificationService {
  ...
  public void notifyDutyUsers() {
    ...
    List<Map<String, Object>> events =
        (List<Map<String, Object>>) infoScreenService.getInfoScreenEvents().get("events");
  ...
  // Send a single consolidated message with all duty times for the user, including the link with
  // verification number
  private void sendMessage(
      String telephoneNumber, String username, List<String> dutyTimes, int verificationNumber) {
    String encodedUsername = encodeUsername(username);
    String link =
        "https://saai.wayshare.de/day-off?username="
            + encodedUsername
            + "&verificationNumber="
            + verificationNumber;
    ...
    if (dutyTimes.size() == 1) {
      ...
      messageBuilder
          ...
          .append(
              "Wenn du heute zu den Zeiten, zu denen du Dienst hast, nicht in der Schule bist, oder aus anderen Gründen keine Zeit für Alarme hast, klicke bitte auf den folgenden Link:\n")
          .append(link)
  ...
  private void sendLiveTickerMessage(Map<String, List<String>> userDuties) {
    ...
    timetableEvents.append(
        "Users were notified about today's timetable events:\n"); // One line break after header
    ...
    String url =
        "https://saai.wayshare.de:9090/api/signalmessage/liveticker?message=" + urlEncodedMessage;
